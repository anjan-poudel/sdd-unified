...
# Installation script for SDD Unified Framework with Roo Code
# Properly registers agents and slash commands

set -e  # Exit on error

echo "=========================================="
echo "SDD Unified Framework - Roo Code Setup"
echo "=========================================="
echo ""
...
# 3. Determine Roo Code config directory
if [ "$SCOPE" = "global" ]; then
    ROO_CONFIG_DIR="$HOME/.roo-code"
else
    ROO_CONFIG_DIR=".roo-code"
fi

echo "Roo Code config directory: $ROO_CONFIG_DIR"
mkdir -p "$ROO_CONFIG_DIR/agents" "$ROO_CONFIG_DIR/commands"

# 4. Register agents
echo ""
echo "Registering SDD agents with Roo Code..."
AGENTS=("sdd-ba" "sdd-architect" "sdd-pe" "sdd-le" "sdd-coder")

for agent in "${AGENTS[@]}"; do
    cp "sdd_unified/agents/configs/${agent}.yaml" "$ROO_CONFIG_DIR/agents/"
...
cp sdd_unified/commands/slash/*.yaml "$ROO_CONFIG_DIR/commands/"
...
HELPER_FILE="$ROO_CONFIG_DIR/sdd_quickstart.md"
cat > "$HELPER_FILE" << 'EOL'
...
echo "=========================================="
echo ""
echo "To start using the framework in Roo Code:"
echo "1. Restart or reload your Roo Code environment"
