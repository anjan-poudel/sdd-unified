...
sdd-unified introduces a **formal context manifest** that travels with each feature, providing:
...
## Overlay Extensions (Pair + Review Mode)

When using the pair+formal-review overlay, add the following fields:
...
```json
{
  ...
  "pair_session": {
    ...
    "session_summary": "Challenger identified coupling risk; design revised."
  },
  "artifact_dri": "sdd-pe",
  "traceability": {
...
These fields preserve accountability and requirement-to-test lineage while keeping the existing context model.
