...
The workflow engine is the heart of sdd-unified's orchestration. It defines HOW tasks flow through the development process using a Directed Acyclic Graph (DAG) structure.
...
### 4. Overlay Compatibility: Pair Sessions

The pair+review overlay does not require a new DAG. It augments task execution metadata:

- task still transitions by dependency status (`PENDING` -> `READY` -> `RUNNING` -> `COMPLETED`/`FAILED`)
- `driver_agent` produces output
- `challenger_agent` critiques assumptions before task lock
- formal independent review remains downstream gate

Result: execution behavior improves while preserving existing workflow structure.
...
sdd-unified assumes the agentic tool (Claude Code/Roo Code) can:
