...
│
├── runtime/adapters/          # Backend adapters for different AI coding tools
│   ├── claude_code.py
│   ├── codex.py
│   ├── gemini.py
│   └── abstract.py
...
Overlays are stacked as decorators around the core task dispatch in the following order (highest priority first):

1. **HIL overlay** (always loaded if enabled)
2. **Evidence policy gate** (if enabled for task)
3. **Agentic review overlay** (if enabled for task)
4. **Paired workflow overlay** (if enabled for task)
5. **Confidence loop overlay** (if enabled for task)
6. **Agent execution**

Each overlay is activated by a flag in the workflow task definition or the project config.

### 2.5 Workflow YAML Schema (Draft)
...
**Goal:** All overlay types are independently configurable and composable.
...
| P2-D5 | Evidence policy gate | Gate using acceptance evidence, verification results, operational readiness |
| P2-D6 | Enhanced HIL overlay | Deadlock detection, loop escape escalation to human |
| P2-D7 | Overlay config schema | Unified schema for enabling/configuring all overlays |
| P2-D8 | Overlay interaction tests | Validate behaviour when multiple overlays are active together |
...
- Given evidence policy gate enabled, confidence percentage alone does not promote; gate uses acceptance evidence, verification results, operational readiness.
- Risk tiers: T0 lightweight, T1 standard, T2 strict + human sign-off.

### Overlay Complexity Management

- Confidence scoring mechanism must provide a toggle to fall back to simple raw evaluation scores to prevent configuration from becoming unmanageable for small teams.
...
| Confidence scoring adds unnecessary complexity | Raw metric mode supported; confidence off by default |
...
- Planning/task-breakdown policy is constitution-driven, not hardcoded.
...
- Framework is installable into an existing project without modifying the project's structure.
